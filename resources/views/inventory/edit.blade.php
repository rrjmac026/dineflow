                    ...
                    <form method="POST" action="{{ route('inventory.update', $inventory) }}" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="item_name" class="block mt-1 w-full" type="text" name="item_name" :value="old('item_name', $inventory->item_name)" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="supplier" class="block mt-1 w-full" type="text" name="supplier" :value="old('supplier', $inventory->supplier)" required />
                                <x-input-error :messages="$errors->get('supplier')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="unit_cost" :value="__('Unit Cost')" />
                                <x-text-input id="unit_cost" class="block mt-1 w-full" type="number" name="unit_cost" step="0.01" :value="old('unit_cost', $inventory->unit_cost)" />
                                <x-input-error :messages="$errors->get('unit_cost')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="reorder_level" :value="__('Reorder Level')" />
                                <x-text-input id="reorder_level" class="block mt-1 w-full" type="number" name="reorder_level" :value="old('reorder_level', $inventory->reorder_level)" required />
