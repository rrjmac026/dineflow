        ...
        <div class="relative min-h-screen flex flex-col items-center justify-center px-4">
            ...
            @if (Route::has('login'))
                <div class="fixed top-0 right-0 p-6 glass-effect rounded-bl-2xl z-50">
                    <nav class="space-x-4">
                        @auth
                            <a href="{{ url('/dashboard') }}" class="font-semibold text-white hover:text-amber-300 transition-colors duration-200">
                                <i class="fas fa-tachometer-alt mr-2"></i>Dashboard
                            </a>
                        @else
                            <a href="{{ route('login') }}" class="font-semibold text-white hover:text-amber-300 transition-colors duration-200">
                                <i class="fas fa-sign-in-alt mr-2"></i>Log in
                            </a>

                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="font-semibold text-white hover:text-amber-300 transition-colors duration-200">
