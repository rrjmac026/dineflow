<x-app-layout>
    ...
    <div class="flex justify-end px-8 mt-4">
        <a href="{{ route('reservations.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 dark:bg-amber-500 dark:hover:bg-amber-600 rounded-lg shadow-sm hover:shadow transition-all duration-200">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full table-auto">
                            <thead class="bg-amber-50 dark:bg-amber-900/20 border-b border-amber-200 dark:border-amber-700">
                                <tr>
                                    <th class="py-3 px-6 text-left text-sm font-medium text-amber-900 dark:text-amber-100">Customer</th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse($reservations as $reservation)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-amber-800/50 transition-colors duration-150">
                                        <td class="py-4 px-6 text-sm text-gray-700 dark:text-gray-200">{{ $reservation->user->name }}</td>
                                        <td class="py-4 px-6 text-sm text-gray-700 dark:text-gray-200">{{ $reservation->date->format('M d, Y') }}</td>
                                        <td class="py-4 px-6 text-sm text-gray-700 dark:text-gray-200">{{ $reservation->time->format('h:i A') }}</td>
                                        <td class="py-4 px-6 text-sm text-gray-700 dark:text-gray-200">{{ $reservation->guests }}</td>
