<x-app-layout>
    ...
    <div class="flex justify-end px-8 mt-4">
        <a href="{{ route('customers.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 dark:bg-amber-500 dark:hover:bg-amber-600 rounded-lg shadow-sm hover:shadow transition-all duration-200">
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse($customers as $customer)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-amber-800/50 transition-colors duration-150">
                                        <td class="py-4 px-6 text-sm text-gray-700 dark:text-gray-200">{{ $customer->name }}</td>
                                        <td class="py-4 px-6 text-sm text-gray-700 dark:text-gray-200">{{ $customer->email }}</td>
                                        <td class="py-4 px-6 text-sm text-gray-700 dark:text-gray-200">{{ $customer->created_at->format('M d, Y') }}</td>
